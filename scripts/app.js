...
(function() {
  ...
  app.getForecast = function(key, label) {
    ...
    // 1- first display old cached data.
    // 2- second Fetch the latest data. If there is no connection we still see something
  ...
  If there is information that you need to persist and reuse across restarts, 
  ...
  - register('/sw.js') This means that the service worker's scope will be the entire origin. 
    In other words, this service worker will receive fetch events for everything on this domain. 
    If we register the service worker file at /example/sw.js, 
    then the service worker would only see fetch events for pages 
